...
│   ├── scripts/getnet-sandbox-check.mjs  # ⚠️ WIP — diagnóstico contra el sandbox real de Getnet (`npm run test:getnet-sandbox`), ver docs/GETNET-SANDBOX-TESTING.md
...
│       └── migration-getnet-wip.sql                     # ⚠️ WIP, NO corrida — pedidos.getnet_payment_id, ver §13 y docs/GETNET-INTEGRACION.md
│
├── docs/
│   ├── ANDROID-BUILD.md       # Guía paso a paso para el builder con Android Studio
│   ├── GETNET-INTEGRACION.md  # ⚠️ WIP — arquitectura, qué se confirmó vs. qué falta contra la API real, checklist de negocio
│   └── GETNET-SANDBOX-TESTING.md  # ⚠️ WIP — plan de pruebas paso a paso para el día que haya credenciales de sandbox
...
> ⚠️ **Alerta para IA:** MercadoPago es la pasarela de pago actual e integrada. El foco de una posible migración pasó de **Payway** a **Getnet** (adquirente de Santander) el 2026-08-24 — el usuario trajo una comparativa interna de tarifas y confirmó que quiere avanzar con Getnet. Payway queda en pausa (esqueleto intacto en `work/2026-08-18-payway-integracion`, sin tocar); Getnet tiene su propio esqueleto WIP inerte en `work/2026-08-24-getnet-integracion`, sin mergear a `main` y sin conectar al flujo real de pedidos — ver `PASARELAS-DE-PAGO.md`, `docs/GETNET-INTEGRACION.md` y `docs/GETNET-SANDBOX-TESTING.md`. Aunque el usuario ya definió la dirección, **activar esto de verdad (credenciales reales, conectar `cliente.js`/`pago.html`) sigue siendo una decisión aparte** que involucra a Fabri (relación con Santander) — no asumir que MercadoPago dejó de ser la pasarela real, no conectar Getnet al flujo de producción por cuenta propia sin confirmar primero.
>
...
| 4 | Getnet vs. MercadoPago — a cargo de Fabri (relación con Santander), no conectar al flujo real sin que él avance. **2026-08-24/25:** el usuario trajo una comparativa interna de Santander/Getnet, confirmó que quiere avanzar con Getnet (el foco pasó de Payway, que queda en pausa) y pidió preparar todo el terreno técnico. Ya está listo: esqueleto WIP inerte completo en `work/2026-08-24-getnet-integracion` (`docs/GETNET-INTEGRACION.md`), script de diagnóstico de sandbox (`backend/scripts/getnet-sandbox-check.mjs`, `npm run test:getnet-sandbox`) y plan de pruebas manual (`docs/GETNET-SANDBOX-TESTING.md`) — nada de esto conectado al flujo real ni mergeado a `main`. **Falta 100% de gestión, no código:** cuenta comercial de Getnet + acceso al developer portal para confirmar los endpoints técnicos (ver checklist en `docs/GETNET-INTEGRACION.md` §4) — ver `PASARELAS-DE-PAGO.md` §3c/§4 y `IMPORTANTE-PREGUNTAS-GERARDO.md` ítem 6. | Pagos |
...
| 8 | Firebase/FCM para push nativo — **reabierto 2026-08-19** a pedido del usuario (antes estaba en pausa explícita, ver más abajo). No está confirmado si ya existe un proyecto de Firebase de un intento anterior (hay un indicio real: la API key huérfana del ítem 6, de un snippet de FCM comentado y borrado) — el usuario no está seguro, hace falta entrar a `console.firebase.google.com` con la cuenta de Google del proyecto y verificar. Implementación real (Firebase Admin SDK en `pushController.js` para FCM API v1, tabla nueva para tokens nativos, `google-services.json` en `android/app/`) pospuesta a propósito hasta terminar el checklist de Play Console del ítem 1. | App nativa / push notifications |
