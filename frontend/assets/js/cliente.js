...
function cargarMapaCarrito(lat,lng){
  ...
  if(!wrap||!div||!window.L)return;
  wrap.style.display='block';
  if(_mapaListo&&_mapaObj&&_pinObj){
    _mapaObj.setView([lat,lng],16);
    _pinObj.setLatLng([lat,lng]);
    return;
  }
  _mapaListo=true;
  _mapaObj=L.map(div,{center:[lat,lng],zoom:16,zoomControl:true,attributionControl:false});
  L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19}).addTo(_mapaObj);
  ...
  _pinObj.on('dragend',async()=>{
    ...
    ubicacionActual=dir;
    selDireccion('gps');
    let numInput=document.getElementById('num-casa-wrap');
    if(!numInput){
      numInput=document.createElement('div');numInput.id='num-casa-wrap';
      numInput.style.cssText='margin-top:8px;display:flex;gap:8px;align-items:center;';
      numInput.innerHTML='<input id="num-casa" placeholder="Numero de puerta (ej: 450)" style="flex:1;border:1.5px solid #FF6B35;border-radius:10px;padding:10px 14px;font-size:13px;outline:none;font-family:inherit;" oninput="actualizarNumCasa()"/>';
      document.getElementById('dir-opt-gps').after(numInput);
    }
    numInput.style.display='flex';
  });
  _mapaObj.on('click',(e)=>{
    _pinObj.setLatLng(e.latlng);
    _pinObj.fire('dragend');
  });
}
...
async function reportarProblema(tipo){
  ...
  try{
    ...
    const{data,error}=await sb.from('reportes').insert([{
      ...
      comercio_id:currentComercio?.id||null,
      tipo,
      estado:'pendiente',
    ...
    if(error)throw error;
    reporteId=data?.id;
    await sb.from('chat_reportes').insert([{reporte_id:reporteId,pedido_id:currentPedido?.id,comercio_id:currentComercio?.id,de:'sistema',texto:`Reporte: "${tipoLabel}". El comercio tiene 10 minutos para resolver.`}]);
    await sb.from('advertencias_comercio').insert([{comercio_id:currentComercio?.id,motivo:tipo,pedido_id:currentPedido?.id}]);
  }catch(e){console.error('[Reporte] Error creando reporte:',e?.message);}
...
async function cargarPedidos(){
  ...
  const estadoConfig={entregado:{clase:'ep-entregado',label:'Entregado',icon:ICONS.check},cancelado:{clase:'ep-cancelado',label:'Cancelado',icon:ICONS.close},en_camino:{clase:'ep-camino',label:'En camino',icon:ICONS.scooter},preparando:{clase:'ep-camino',label:'Preparando',icon:ICONS.plate},nuevo:{clase:'ep-camino',label:'Confirmado',icon:ICONS.confetti}};
    container.innerHTML=data.map(p=>{
      ...
      const itemsStr=items.map(i=>`${i.qty||i.cantidad||1}x ${_escHtml(i.nombre||i.name||'—')}`).join(', ');
  ...
  return`<div class="ped-card" onclick="${activo?`retomarTracking('${p.id}')`:`verDetallePedido('${p.id}')`}"><div class="ped-top"><div class="ped-name">${est.icon} ${_escHtml(p.comercios?.nombre||'Comercio')}</div><span class="estado-pill ${est.clase}">${est.label}</span></div><div style="font-size:12px;color:var(--gray-600);margin-bottom:6px;line-height:1.5;">${itemsStr||'Sin detalle'}</div><div style="display:flex;justify-content:space-between;align-items:center;"><div class="ped-meta">${ICONS.calendar} ${fecha} · ${ICONS.clock} ${hora}</div><div style="font-size:14px;font-weight:800;color:var(--brand);">$${Number(p.total||0).toLocaleString('es-AR')}</div></div>${p.notas?`<div style="margin-top:6px;font-size:11px;color:var(--gray-400);background:var(--gray-50);border-radius:8px;padding:6px 10px;">${ICONS.chat} ${_escHtml(p.notas)}</div>`:''}${chatBtn}${repeatBtn}</div>`;
    }).join('');
  }catch{
    if(currentPedido){container.innerHTML=`<div class="ped-card" onclick="go('tracking')"><div class="ped-top"><div class="ped-name">${_escHtml(currentComercio?.nombre||'Tu pedido')}</div><span class="estado-pill ep-camino">En camino</span></div><div class="ped-meta">$${Number(currentPedido.total||0).toLocaleString('es-AR')} · Hoy</div></div>`;}
...
async function cargarDetallePedido(pedidoId){
  ...
  try{
    ...
    const est=estadoConfig[p.estado]||estadoConfig.nuevo;
    cont.innerHTML=`<div style="background:#fff;border-radius:var(--radius-lg);overflow:hidden;border:1px solid var(--gray-100);margin-bottom:14px;"><div style="padding:16px;border-bottom:1px solid var(--gray-100);"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;"><div style="font-size:16px;font-weight:800;color:var(--black);">${_escHtml(p.comercios?.nombre||'Comercio')}</div><span style="font-size:12px;font-weight:700;padding:5px 12px;border-radius:20px;background:${est.bg};color:${est.color};">${est.label}</span></div><div style="font-size:12px;color:var(--gray-400);">${fecha} · ${hora} hs · Pedido #${p.numero||'—'}</div></div><div style="padding:16px;border-bottom:1px solid var(--gray-100);"><div style="font-size:11px;font-weight:700;color:var(--brand);text-transform:uppercase;letter-spacing:.06em;margin-bottom:10px;">Productos</div>${items.map(i=>`<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid var(--gray-50);"><span style="font-size:13px;color:var(--black);">${i.qty||1}x ${_escHtml(i.nombre)}</span><span style="font-size:13px;font-weight:700;color:var(--black);">$${Number((i.precio||0)*(i.qty||1)).toLocaleString('es-AR')}</span></div>`).join('')}</div><div style="padding:16px;border-bottom:1px solid var(--gray-100);"><div style="display:flex;justify-content:space-between;font-size:13px;padding:4px 0;color:var(--gray-600);"><span>Subtotal</span><span>$${Number(p.subtotal||p.total-(p.costo_envio||0)||0).toLocaleString('es-AR')}</span></div><div style="display:flex;justify-content:space-between;font-size:13px;padding:4px 0;color:var(--gray-600);"><span>Envío</span><span>$${Number(p.costo_envio||0).toLocaleString('es-AR')}</span></div><div style="display:flex;justify-content:space-between;font-size:16px;font-weight:800;padding:8px 0 0;border-top:1.5px solid var(--gray-200);margin-top:4px;"><span>Total</span><span style="color:var(--brand);">$${Number(p.total||0).toLocaleString('es-AR')}</span></div></div><div style="padding:16px;"><div style="font-size:12px;color:var(--gray-400);margin-bottom:4px;">Método de pago</div><div style="font-size:13px;font-weight:600;">${metodoIcon[p.metodo_pago]||p.metodo_pago||'—'}</div>${p.direccion_entrega?`<div style="margin-top:10px;"><div style="font-size:12px;color:var(--gray-400);margin-bottom:4px;">Dirección de entrega</div><div style="font-size:13px;font-weight:600;">${_escHtml(p.direccion_entrega)}</div></div>`:''} ${p.notas?`<div style="margin-top:10px;background:var(--gray-50);border-radius:8px;padding:10px 12px;"><div style="font-size:12px;color:var(--gray-400);margin-bottom:2px;">Nota al comercio</div><div style="font-size:13px;">${_escHtml(p.notas)}</div></div>`:''}</div></div>${p.estado==='entregado'?`<button onclick="repetirPedido('${p.comercio_id}')" style="width:100%;background:var(--brand);color:#fff;border:none;border-radius:var(--radius-md);padding:14px;font-size:15px;font-weight:800;cursor:pointer;margin-bottom:10px;">Repetir este pedido</button>`:''}`;
...
window.addEventListener('load', () => {
  ...
  cargarPerfil(user);detectarUbicacion();
...
async function cargarOfertasBanner(){
  try{
    ...
    document.getElementById('ofertas-slides').innerHTML=promos.map(p=>{
      ...
      return`<div onclick="abrirComercio('${p.comercio_id}')" style="width:155px;border-radius:14px;overflow:hidden;background:#111;flex-shrink:0;cursor:pointer;position:relative;box-shadow:0 2px 10px rgba(0,0,0,.15);"><div style="position:relative;height:90px;overflow:hidden;"><img src="${_escHtml(img)}" alt="${_escHtml(comercioNombre)}" style="width:100%;height:90px;object-fit:cover;display:block;filter:brightness(.45);"/><div style="position:absolute;inset:0;background:linear-gradient(to top,rgba(0,0,0,.7) 0%,rgba(0,0,0,0) 55%);"></div><div style="position:absolute;top:8px;left:8px;background:#FF6B35;color:#fff;font-size:8px;font-weight:800;padding:2px 8px;border-radius:20px;letter-spacing:.04em;">${_escHtml(badgeLabel)}</div></div><div style="padding:8px 10px 10px;background:#111;"><div style="font-size:12px;font-weight:700;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${_escHtml(comercioNombre)}</div><div style="font-size:10px;color:rgba(255,255,255,.5);margin-top:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${_escHtml(descSub)}</div></div></div>`;
    }).join('');
...
async function cargarNuevosComercios(){
  ...
  try{
    ...
    slides.innerHTML=items.map((c,i)=>{
      ...
      return`<div onclick="abrirComercio('${c.id}')" class="tarjeta-com" style="width:130px;border-radius:14px;overflow:hidden;background:var(--white);flex-shrink:0;cursor:pointer;box-shadow:var(--shadow-md);border:1px solid var(--gray-100);animation:cardIn .32s ease both;animation-delay:${i*0.06}s;"><div style="position:relative;height:90px;overflow:hidden;"><img src="${_escHtml(img)}" alt="${_escHtml(c.nombre)}" loading="lazy" style="width:100%;height:90px;object-fit:cover;display:block;"/><div style="position:absolute;inset:0;background:linear-gradient(to top,rgba(0,0,0,.28) 0%,transparent 55%);pointer-events:none;"></div>${esNuevo?`<div class="badge-nuevo-anim" style="position:absolute;top:6px;left:6px;background:#16A34A;color:#fff;font-size:8px;font-weight:800;padding:2px 7px;border-radius:20px;letter-spacing:.04em;">NUEVO</div>`:''}
      </div><div style="padding:8px 9px 10px;"><div style="font-size:12px;font-weight:700;color:var(--black);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${_escHtml(c.nombre)}</div><div style="font-size:10px;color:var(--gray-400);margin-top:2px;text-transform:capitalize;">${_escHtml(c.categoria||'')}</div><div style="font-size:10px;font-weight:700;color:#F59E0B;margin-top:3px;">★ ${rating}</div></div></div>`;
    }).join('');
