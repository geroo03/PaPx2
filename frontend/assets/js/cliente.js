...
async function abrirRubro(catId,label){
  ...
  const envios={comida:'$800',carniceria:'$600',farmacia:'$700',supermercado:'$1.000',verduleria:'$800',panaderia:'$600',heladeria:'$600',bebidas:'$500',mascotas:'$700'};
...
async function abrirComercio(id){const com=allComercios.find(c=>c.id===id);if(!com)return;if(!com.abierto_ahora){showToast('Este comercio está cerrado ahora');return;}currentComercio=com;window.state.cart={};document.getElementById('det-name').textContent=com.nombre;document.getElementById('det-meta').textContent=`⭐ ${com.rating} · ${com.total_pedidos||0} pedidos · Envío desde $600`;document.getElementById('cart-comercio-name').textContent=com.nombre;document.getElementById('cart-float').style.display='none';document.getElementById('ratings-comercio').style.display='none';go('detail');cargarRatingsComercio(id);try{const{data,error}=await window.sb.from('productos').select('*').eq('comercio_id',id);if(error){console.error('[PaP] Error cargando productos:',error.message);document.getElementById('menu-container').innerHTML='<div class="empty"><div class="big">⚠️</div><p>Error al cargar el menú. Intentá de nuevo.</p></div>';return;}if(!data||!data.length){document.getElementById('menu-container').innerHTML='<div class="empty"><div class="big">🍽️</div><p>Este comercio no tiene productos cargados todavía.</p></div>';return;}const cats=[...new Set(data.map(p=>p.categoria_id||p.categoria||'General'))];document.getElementById('menu-container').innerHTML='<div class="section-card">'+cats.map(cat=>`<div class="menu-cat-label">${cat}</div>${data.filter(p=>(p.categoria_id||p.categoria||'General')===cat).map(p=>{const precio=Number(p.precio_base??p.precio??0);const img=p.imagen_url?`<img src="${p.imagen_url}" style="width:54px;height:54px;object-fit:cover;border-radius:10px;flex-shrink:0;margin-left:8px;" loading="lazy"/>`:'';return`<div class="menu-item"><div style="flex:1;min-width:0;"><div class="mi-name">${p.nombre}</div><div class="mi-desc">${p.descripcion||''}</div></div><div class="mi-right">${img}<div class="mi-price">$${precio.toLocaleString('es-AR')}</div><button class="add-btn" onclick="addCart('${p.id}','${p.nombre.replace(/'/g,"\\'")}',${precio})">+</button></div></div>`;}).join('')}`).join('')+'</div>';}catch(e){console.error('[PaP] Excepción al cargar productos:',e);document.getElementById('menu-container').innerHTML='<div class="empty"><div class="big">⚠️</div><p>No se pudo cargar el menú. Revisá tu conexión.</p></div>';}}
...
function selMetodoPago(m){metodoPagoGuardado=m;localStorage.setItem('pap_metodo_pago',m);showToast('✅ Método de pago guardado');abrirPantallaExtra('pagos');}
...
async function detectarUbicacion(){
  ...
  },(err)=>{setUbicacionTexto('📍 Activar ubicación');},{timeout:10000,maximumAge:0,enableHighAccuracy:true});
}
